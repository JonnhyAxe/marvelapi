...
public class MarvelCharacterIdentityService implements CharacterIdentity {
    ...
    public void init(){
        ...
        //
        // int calls = totalCharacters != null ? (int)
        // Math.round(totalCharacters / 100.00) : 0;
        // logger.debug("Executing {} ", calls);
        // if (Objects.nonNull(totalCharacters) && calls > 0) {
        // response.getData().getResults().parallelStream().forEach(c ->
        ...
        // "Through concentration, Hal could merge the images of his brother
        // imprinted on his glasses and thus cause his brother Chuck to reappear
        // as a three-dimensional man, clad in an altered version of his
        // experimental flight suit and endowed with physical abilities roughly
        // three times greater than those of an ordinary human. Hal would fall
        // into a trance-like state when Chuck appeared, and Chuck could only
        // exist in the three-dimensional world for three hours at a time, after
        // which Hal had to revive.\nAs the 3-D Man, Chandler possessed roughly
        // three times the physical abilities and sensory acuity of an ordinary
        // human in peak condition and is capable of slightly superhuman
        // strength and speed. His stamina, durability, agility and reflexes are
        // also estimated to be superhuman, namely roughly triple that of a
        // human in peak physical condition. He could also sense Skrulls no
        // matter what form they took.");
        // logger.debug("MarvelCharacterIdentityService finished with character
        // " + characters.size());
